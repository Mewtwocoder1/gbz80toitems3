...
function compile(evt) {
    ...
    for (i = 0; i < n;) {
        ...
        var pieces = lines[i].toLowerCase()
            .replace('[', '(').replace(']', ')') // Braces will be parsed the same as parentheses.
    ...
    for (i = 0; i < n; i++) {
        var b = byteStream[i];
        var hexValue = decToHex(b).toUpperCase(); // Convert to hex
        
        // Output the raw hex value
        itemList.push(hexValue);
    }
    
    /** END COMPILER **/
    
    var output = itemList.join('<br/>'); // Join hex values with line breaks
